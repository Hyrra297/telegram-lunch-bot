...
- Tab "Tuần này": xem ai đặt, nhập tối đa 5 món cho từng ngày (admin). Mỗi món 1 hàng: **tên món + 2 nút 45k / 50k** (`setQuickPrice()`, nút đang chọn có class `qbtn-on`); bấm lại nút đang sáng = bỏ chọn về mặc định `daily_votes.price` (45k). Ngày thường KHÔNG có ô gõ giá — giá đi kèm qua `<input type="hidden" name="priceN">` nên giá cũ không bị mất khi lưu. **Riêng T6** có thêm ô số nhỏ để gõ giá lẻ (bún đậu 35k), cùng ô **ship**.
